module NewspaperWorks
  ...
  class PublicationDateStartEndValidator < ActiveModel::Validator
    ...
    def validate(record)
      ...
      valid_dates?(record) && start_before_end?(record)
    end
    ...
    private
      ...
      def start_before_end?(record)
        ...
        !record.errors[:publication_date_start].present?
      end

      def valid_dates?(record)
        date_incorrect_msg = "Incorrect Date. Date input should be formatted yyyy[-mm][-dd] and be a valid date."
        record.errors[:publication_date_start] << date_incorrect_msg unless publication_date_valid?(record.publication_date_start)
        record.errors[:publication_date_end] << date_incorrect_msg unless publication_date_valid?(record.publication_date_end)
        record.errors[:publication_date_start].present? || record.errors[:publication_date_end].present?
      end
  end
end
