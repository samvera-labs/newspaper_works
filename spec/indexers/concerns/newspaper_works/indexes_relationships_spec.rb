...
RSpec.describe NewspaperWorks::IndexesRelationships do
  # use a local var so we can create fixtures only once
  # (let() runs on every spec, and Rubocop dislikes @instance vars)
  page_for_indexrel_specs = model_fixtures(NewspaperPage)
  let(:page_indexer) { NewspaperPageIndexer.new(page_for_indexrel_specs) }

  describe '#index_relationships' do
    let(:solr_doc) { {} }
    before { page_indexer.index_relationships(page_for_indexrel_specs, solr_doc) }
  ...
  describe '#index_publication_title' do
    let(:solr_doc) { {} }
    before { page_indexer.index_publication_title(page_for_indexrel_specs, solr_doc) }
  ...
  describe '#index_container' do
    let(:solr_doc) { {} }
    before { page_indexer.index_container(page_for_indexrel_specs, solr_doc) }
  ...
  describe '#index_issue' do
    let(:solr_doc) { {} }
    before { page_indexer.index_issue(page_for_indexrel_specs, solr_doc) }
  ...
  describe '#index_pages' do
    let(:article) { page_for_indexrel_specs.articles.first }
    let(:article_indexer) { NewspaperArticleIndexer.new(article) }
    let(:solr_doc) { {} }
    before { page_indexer.index_pages(article, solr_doc) }
    it 'sets the issue fields correctly' do
      expect(solr_doc['page_ids_ssim'].first).to eq(page_for_indexrel_specs.id)
  ...
  describe '#index_articles' do
    let(:solr_doc) { {} }
    before { page_indexer.index_articles(page_for_indexrel_specs, solr_doc) }
